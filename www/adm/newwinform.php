...
<input type="hidden" name="nw_device" value="both">
                <tr>
                    <th scope="row"><label for="nw_pages">노출 페이지</label></th>
                    <td>
                        <?php echo help("팝업이 표시될 페이지를 선택해주세요. '전체'를 선택하면 모든 페이지에서 노출됩니다."); ?>
                        <?php
                        $page_list = array(
                            'front'     => '회사소개',
                            'contact'   => '창업문의',
                        ...
                        $selected_pages = isset($nw['nw_pages']) ? explode(',', $nw['nw_pages']) : array();

                        // 전체 체크박스
                        $all_checked = in_array('all', $selected_pages) ? 'checked' : '';
                        echo "<label style='margin-right:10px;'>
                                <input type='checkbox' name='nw_pages[]' value='all' {$all_checked} onclick='toggleAllPages(this)'> 전체
                              </label>";

                        foreach ($page_list as $pid => $label) {
                            $checked = in_array($pid, $selected_pages) ? 'checked' : '';
                            echo "<label style='margin-right:10px;'>
                                    <input type='checkbox' name='nw_pages[]' value='{$pid}' class='page_checkbox' {$checked}> {$label}
                                  </label>";
                        }
                        ?>

                        <script>
                            function toggleAllPages(allCheckbox) {
                                const checkboxes = document.querySelectorAll('.page_checkbox');
                                if (allCheckbox.checked) {
                                    checkboxes.forEach(cb => cb.checked = false);
                                }
                            }

                            // 개별 페이지 체크 시 전체 체크 해제
                            const pageCheckboxes = document.querySelectorAll('.page_checkbox');
                            pageCheckboxes.forEach(cb => {
                                cb.addEventListener('change', function() {
                                    if(this.checked){
                                        document.querySelector('input[name="nw_pages[]"][value="all"]').checked = false;
                                    }
                                });
                            });
                        </script>
                ...
                <tr>
                    <th scope="row"><label for="nw_subject">팝업 노출 순서<strong class="sound_only"> 필수</strong></label></th>
                    <td>
                        <input min="0" max="999" type="number" name="nw_order" value="<?php echo get_sanitize_input($nw['nw_order']); ?>" id="nw_order" required class="frm_input required" size="5">
...
<script>
    function frmnewwin_check(f) {
        ...
        // ✅ 페이지 체크 유효성 검사 추가
        const pageCheckboxes = document.querySelectorAll('input[name="nw_pages[]"]');
        let pageChecked = false;
        pageCheckboxes.forEach(cb => {
            if (cb.checked) pageChecked = true;
        });

        if (!pageChecked) {
            alert("노출 페이지는 최소 한 개 이상 선택해야 합니다.");
            return false;
        }
